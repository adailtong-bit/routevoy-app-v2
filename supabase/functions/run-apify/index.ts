  ...
  try {
    ...
    try {
      ...
      if (url && url !== 'all') {
        ...
        const searchQ = query ? `${query} -job -jobs -career` : 'deals discounts coupon travel hotel rental -job -jobs -career';
        ...
        if (data && data.organic_results) {
          ...
          data.organic_results.forEach((r: any) => {
            if (!isJobRelated(r.title + ' ' + (r.snippet || ''))) {
              extractedItems.push({
                title: r.title,
      ...
      try {
        ...
        searchFormData.append('q', `${query || 'deals discount travel hotel coupon'} (deal OR discount OR coupon) -job -jobs -career`)
        ...
        if (ddgRes.ok) {
          ...
          $('.result').each((i, el) => {
            ...
            if (title && link.startsWith('http') && !link.includes('duckduckgo.com') && !isJobRelated(title + ' ' + snippet)) {

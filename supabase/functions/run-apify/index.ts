...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    addLog(`Received request: engine=${engine}, url=${url}, query=${query}, limit=${limit}`)

    const SCRAPER_API_KEY = Deno.env.get('SCRAPER_API_KEY') || '124578ab1a147cdc8baf7376968c4f1f'
    ...
    try {
      ...
      if (url && url !== 'all') {
        ...
        $('article, .card, [class*="product"], [class*="offer"]').each((i, el) => {
           ...
           if(itemTitle && itemLink) {
             if (!itemLink.startsWith('http')) {
               try { itemLink = new URL(itemLink, url).toString(); } catch(e) {}
             }
             extractedItems.push({
               title: itemTitle,
        ...
        if(extractedItems.length === 0 && title && url) {
           extractedItems.push({ title, description, imageUrl, productLink: url, storeName: new URL(url).hostname });
        }
      } else {
        const searchQ = query ? `${query} inurl:deal OR inurl:sale OR inurl:product -job -jobs -career -hiring` : '"deals" OR "discounts" OR "coupon" OR "hotel" (inurl:deal OR inurl:sale OR inurl:product) -job -jobs -career -hiring';
        ...
        addLog(`Received JSON response from ScraperAPI. Keys: ${Object.keys(data).join(',')}`)
        
        if (data && data.organic_results) {
          addLog(`Found ${data.organic_results.length} organic results.`)
          
          const isValidDeal = (title: string, link: string, snippet: string) => {
            const t = (title + ' ' + snippet).toLowerCase();
            const blacklist = ['vaga', 'emprego', 'job ', 'jobs', 'career', 'hiring', 'trabalhe', 'carreira', 'recruitment', 'recrutamento', 'salary', 'salário', 'resume', 'glassdoor', 'indeed', 'linkedin'];
      ...
      try {
        ...
        searchFormData.append('q', `${query || 'deals discount travel hotel coupon us usd'} (deal OR discount OR coupon OR sale) -job -jobs -career -hiring`)
        ...
        if (ddgRes.ok) {
          ...
          if (title && link.startsWith('http') && !link.includes('duckduckgo.com') && isValidDeal(title, link, snippet)) {
            const priceMatch = snippet.match(/(?:\$|€|£|R\$)\s*\d+(?:[.,]\d{2})?/)
            let priceStr = priceMatch ? priceMatch[0] : null;
              
            if (priceStr) {
              const pVal = parseFloat(priceStr.replace(/[^\d.,]/g, '').replace(',', '.'));
    ...
    if (finalItems.length > 0) {
      ...
      newItems = uniqueFinalItems.filter(item => !existingSet.has(item.unique_hash));
      ...
      if (newItems.length > 0) {
        ...
        if (error) {
          addLog(`Critical Insert Error: ${error.message} (Code: ${error.code})`)
          if (error.code !== '23505') { // If not duplicate key

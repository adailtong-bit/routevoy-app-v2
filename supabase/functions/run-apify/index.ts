...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      if (url && url !== 'all') {
        ...
        if (contentType.includes('application/json')) {
          ...
          $('.g').each((i, el) => {
            ...
            const description = $(el).find('.VwiC3b, .s3v9rd, .IsZvec').first().text().trim();
      ...
      console.warn("[run-apify] External API failed:", fetchError.message);
      
      // Fallback robusto para busca orgânica (sem dados fake)
      try {
        const searchFormData = new URLSearchParams()
        searchFormData.append('q', `${query || 'oferta desconto'} (oferta OR cupom)`)
        ...
        if (ddgRes.ok) {
          ...
          $('.result').each((i, el) => {
            if (extractedItems.length >= limit) return
            ...
            if (title && link.startsWith('http') && !link.includes('duckduckgo.com')) {
              ...
              extractedItems.push({
                ...
                imageUrl: null // Garantindo que não retornaremos imagens geradas/fake
              })
            }
          })
        }
      } catch (ddgErr: any) {
        console.warn("[run-apify] Organic Search fallback also failed:", ddgErr.message)
      }
    }
    ...
    if (finalItems.length > 0) {
      ...
      if (newItems.length > 0) {
        ...
        if (error) {
          console.error("[run-apify] Erro Crítico ao inserir items no banco:", error);

...
DECLARE
  new_user_id uuid;
BEGIN
  -- Insert seed admin user if not exists
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'adailtong@gmail.com') THEN
    ...
    VALUES (new_user_id, 'adailtong@gmail.com', 'Administrador', 'admin')
    ON CONFLICT (id) DO NOTHING;
  END IF;
END $$;

-- Ensure RLS allows the admin to SELECT, UPDATE, DELETE all ad_campaigns

-- Ensure profiles SELECT allows same company_id visibility
...
-- Ensure merchants/managers can UPDATE profiles in their company
...
-- Ensure merchants/managers can DELETE (remove from company) profiles in their company
...
-- user_invitations: SELECT for same company_id
...
-- user_invitations: INSERT for same company_id
...
-- Seed verification: ensure adailtong@gmail.com has company_id and admin role
DO $$
DECLARE
  v_user_id uuid;
  v_merchant_id text;
BEGIN
  ...
  -- Upsert profile with company_id and admin role

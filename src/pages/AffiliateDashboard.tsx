...
export default function AffiliateDashboard() {
  ...
  const fetchData = async (forceSync = false) => {
    ...
    try {
      ...
      const isMasterUser = role === 'super_admin' || role === 'admin'
  ...
  const isPending =
    !isMaster &&
    (partner?.status === 'pending' || profile?.status === 'pending')
  ...
  return (
    <div className="container max-w-7xl py-8 animate-fade-in-up">
      <div className="flex items-center justify-between border-b pb-4 mb-8">
        ...
        >
          {!isPending
            ? t('affiliate.status_active', 'Status da Conta: Ativo')
            : t('affiliate.status_pending', 'Status da Conta: Pendente')}
        </Badge>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0 relative">
          {isPending && activeTab !== 'overview' && (
            <div className="absolute inset-0 z-50 bg-white/70 backdrop-blur-sm flex flex-col items-center pt-24 p-6 text-center rounded-xl animate-in fade-in zoom-in-95 duration-200">
              <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md border border-slate-100 flex flex-col items-center">
                <div className="w-16 h-16 bg-amber-100 text-amber-600 flex items-center justify-center rounded-full mb-4">
                  <AlertCircle className="w-8 h-8" />
                </div>
                <h2 className="text-2xl font-bold text-slate-800 mb-3">
                  {t('affiliate.restricted_access', 'Acesso Restrito')}
                </h2>
                <p className="text-slate-600 mb-6 leading-relaxed">
                  Please update your profile to continue. Your account is
                  currently pending approval from a franchise manager or
                  administrator.
                </p>
                <Button
                ...
                >
                  {t('affiliate.complete_profile_btn', 'Atualizar Perfil')}
                </Button>
              </div>
            </div>
          )}
          ...
          <div
            className={cn(
              ...
              isPending && activeTab !== 'overview'
                ? 'opacity-20 pointer-events-none select-none blur-sm'
                : '',
            )}
          >
            {activeTab === 'overview' && (
                    ...
                    <div>
                      ...
                      <p className="text-amber-700 mt-1">
                        {t(
                          ...
                          'Sua conta está aguardando aprovação de um administrador ou franqueado para liberar todos os recursos.',
                        )}
                      </p>
                    </div>
                  </div>

...
//     USING: ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR (company_id IN ( SELECT merchants.id    FROM merchants   WHERE (merchants.email = (( SELECT users.email            FROM auth.users           WHERE (users.id = auth.uid())))::text))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.email = (( SELECT users.email            FROM auth.users           WHERE (users.id = auth.uid())))::text))) OR (affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))))

...
//     WITH CHECK: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'franchisee'::text) AND ((profiles.franchise_id = affiliate_partners.franchise_id) OR (affiliate_partners.franchise_id IS NULL)))))
// Table: affiliate_platforms
...
//     WITH CHECK: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'franchisee'::text) AND ((profiles.franchise_id = merchants.franchise_id) OR (merchants.franchise_id IS NULL)))))

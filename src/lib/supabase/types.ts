    ...
    Tables: {
      ...
      crm_campaigns: {
        ...
        Relationships: [
          ...
          {
            foreignKeyName: "crm_campaigns_linked_offer_id_fkey"
...
//   FOREIGN KEY crm_campaigns_linked_offer_id_fkey: FOREIGN KEY (linked_offer_id) REFERENCES coupons(id) ON DELETE SET NULL
...
//     USING: ((company_id = (auth.uid())::text) OR (franchise_id = (auth.uid())::text) OR ((affiliate_id)::text = (auth.uid())::text) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR (company_id IN ( SELECT merchants.id    FROM merchants   WHERE (merchants.email = (( SELECT users.email            FROM auth.users           WHERE (users.id = auth.uid())))::text))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.email = (( SELECT users.email            FROM auth.users           WHERE (users.id = auth.uid())))::text))) OR ((affiliate_id)::text IN ( SELECT (affiliate_partners.id)::text AS id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))))

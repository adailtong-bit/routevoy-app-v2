...
//     WITH CHECK: ((company_id = ( SELECT profiles.company_id    FROM profiles   WHERE (profiles.id = auth.uid())  LIMIT 1)) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))))

...
//     WITH CHECK: true
//   Policy "Enable select for all" (SELECT, PERMISSIVE) roles={public}
//     USING: true
...
//   Policy "hierarchy_ads_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_hierarchy_access('ad_campaigns'::text, franchise_id, (company_id)::text, NULL::uuid, NULL::uuid)
//     WITH CHECK: check_hierarchy_access('ad_campaigns'::text, franchise_id, (company_id)::text, NULL::uuid, NULL::uuid)
//   Policy "hierarchy_ads_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((status = 'active'::text) OR check_hierarchy_access('ad_campaigns'::text, franchise_id, (company_id)::text, NULL::uuid, NULL::uuid))
...
//   Policy "admin_insert_affiliates" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR ((auth.jwt() ->> 'email'::text) = 'adailtong@gmail.com'::text))
//   Policy "affiliate_insert_own" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((user_id = auth.uid()) OR (email = ((current_setting('request.jwt.claims'::text, true))::json ->> 'email'::text)))
//   Policy "affiliate_own" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (user_id = auth.uid())
//     WITH CHECK: (user_id = auth.uid())
//   Policy "affiliate_update_own" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (user_id = auth.uid())
//     WITH CHECK: (user_id = auth.uid())
//   Policy "auth_read_own_affiliate" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((user_id = auth.uid()) OR (franchise_id IN ( SELECT profiles.franchise_id    FROM profiles   WHERE (profiles.id = auth.uid()))))
//   Policy "authenticated_insert_affiliate_partners" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "authenticated_update_affiliate_partners" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
//   Policy "ensure_hierarchy_select_affiliates" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND (p.franchise_id = affiliate_partners.franchise_id)))) OR (user_id = auth.uid()))
...
//   Policy "hierarchy_affiliates_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_hierarchy_access('affiliate_partners'::text, franchise_id, NULL::text, id, user_id)
//   Policy "hierarchy_affiliates_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: check_hierarchy_access('affiliate_partners'::text, franchise_id, NULL::text, id, user_id)
// Table: affiliate_platforms
//   Policy "admin_all_affiliate_platforms" (ALL, PERMISSIVE) roles={public}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])))))
//   Policy "authenticated_select_platforms" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
...
//   Policy "hierarchy_coupons_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_hierarchy_access('coupons'::text, franchise_id, (company_id)::text, NULL::uuid, user_id)
//     WITH CHECK: check_hierarchy_access('coupons'::text, franchise_id, (company_id)::text, NULL::uuid, user_id)
//   Policy "hierarchy_coupons_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((status = 'active'::text) OR check_hierarchy_access('coupons'::text, franchise_id, (company_id)::text, NULL::uuid, user_id))
...
//     USING: (status = 'active'::text)
// Table: crawler_logs
//   Policy "affiliates_insert_crawler_logs" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
//   Policy "affiliates_select_crawler_logs" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
...
//     USING: true
// Table: crawler_sources
//   Policy "affiliates_delete_crawler_sources" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
//   Policy "affiliates_insert_crawler_sources" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
//   Policy "affiliates_select_crawler_sources" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
//   Policy "affiliates_update_crawler_sources" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
//     WITH CHECK: ((affiliate_id IN ( SELECT affiliate_partners.id    FROM affiliate_partners   WHERE (affiliate_partners.user_id = auth.uid()))) OR (franchise_id IN ( SELECT franchises.id    FROM franchises   WHERE (franchises.owner_id = (auth.uid())::text))) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = ANY (ARRAY['admin'::text, 'super_admin'::text])))
...
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])))))
//   Policy "admin_all_franchises_override" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR ((auth.jwt() ->> 'email'::text) = 'adailtong@gmail.com'::text))
//   Policy "auth_read_franchise_by_profile" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (id IN ( SELECT profiles.franchise_id    FROM profiles   WHERE (profiles.id = auth.uid())))
//   Policy "ensure_hierarchy_select_franchises" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.franchise_id = franchises.id)))))
...
//   Policy "hierarchy_franchises_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "hierarchy_franchises_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
...
//     USING: ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR ((auth.jwt() ->> 'email'::text) = 'adailtong@gmail.com'::text))
//   Policy "authenticated_delete_merchants" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "authenticated_insert_merchants" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "authenticated_select_merchants" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "authenticated_update_merchants" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
...
//   Policy "hierarchy_merchants_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "hierarchy_merchants_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
...
//   Policy "merchants_delete" (DELETE, PERMISSIVE) roles={public}
//     USING: true
//   Policy "merchants_insert" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: true
//   Policy "merchants_select" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "merchants_update" (UPDATE, PERMISSIVE) roles={public}
//     USING: true
...
//     USING: ((auth.uid() = id) OR (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))))
//   Policy "profiles_delete_policy" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: ((auth.uid() = id) OR ((auth.jwt() ->> 'role'::text) = 'admin'::text) OR ((auth.jwt() ->> 'role'::text) = 'super_admin'::text))
//   Policy "profiles_insert_policy" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (auth.uid() = id)
//   Policy "profiles_select_policy" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((auth.uid() = id) OR ((auth.jwt() ->> 'role'::text) = 'admin'::text) OR ((auth.jwt() ->> 'role'::text) = 'super_admin'::text) OR ((auth.jwt() ->> 'role'::text) = 'franchisee'::text))
//   Policy "profiles_update_policy" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: ((auth.uid() = id) OR ((auth.jwt() ->> 'role'::text) = 'admin'::text) OR ((auth.jwt() ->> 'role'::text) = 'super_admin'::text))
//     WITH CHECK: ((auth.uid() = id) OR ((auth.jwt() ->> 'role'::text) = 'admin'::text) OR ((auth.jwt() ->> 'role'::text) = 'super_admin'::text))
// Table: site_mappings
...
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])))))
//   Policy "authenticated_delete_site_settings" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "authenticated_insert_site_settings" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "authenticated_update_site_settings" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true

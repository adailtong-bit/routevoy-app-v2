...
//     USING: (EXISTS ( SELECT 1    FROM franchises f   WHERE ((f.email = (( SELECT users.email            FROM auth.users           WHERE (users.id = auth.uid())))::text) AND (profiles.company_id IN ( SELECT m.id            FROM merchants m           WHERE ((m.region_id = f.region_id) OR (m.region = f.region)))))))

...
//   Policy "ensure_hierarchy_select_profiles" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))) OR (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND (p.franchise_id = profiles.franchise_id)))))
...
//   Policy "hierarchy_profiles_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((id = auth.uid()) OR check_hierarchy_access('profiles'::text, franchise_id, company_id, NULL::uuid, id))
//     WITH CHECK: ((id = auth.uid()) OR check_hierarchy_access('profiles'::text, franchise_id, company_id, NULL::uuid, id))
//   Policy "hierarchy_profiles_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((id = auth.uid()) OR check_hierarchy_access('profiles'::text, franchise_id, company_id, NULL::uuid, id))
...
//   Policy "profiles_own_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (id = auth.uid())
//   Policy "profiles_own_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (id = auth.uid())
//   Policy "profiles_own_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (id = auth.uid())
//     WITH CHECK: (id = auth.uid())
// Table: site_mappings

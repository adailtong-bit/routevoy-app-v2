...
//     USING: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND ((ad_campaigns.company_id)::text IN ( SELECT m.id            FROM merchants m           WHERE (m.region_id = f.region_id))))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND ((ad_campaigns.company_id)::text IN ( SELECT m.id            FROM merchants m           WHERE (m.region_id = f.region_id))))))
...
//     USING: ((ad_id IN ( SELECT ad_campaigns.id    FROM ad_campaigns   WHERE (((ad_campaigns.company_id)::text IN ( SELECT profiles.company_id            FROM profiles           WHERE ((profiles.id = auth.uid()) AND (profiles.company_id IS NOT NULL)))) OR ((ad_campaigns.company_id)::text = (auth.uid())::text)))) OR (advertiser_id IN ( SELECT ad_advertisers.id    FROM ad_advertisers   WHERE (ad_advertisers.email = (( SELECT users.email            FROM auth.users           WHERE (users.id = auth.uid())))::text))))
// Table: ad_pricing
...
//     USING: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND ((coupons.franchise_id = f.id) OR ((coupons.company_id)::text IN ( SELECT m.id            FROM merchants m           WHERE (m.region_id = f.region_id)))))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND ((coupons.franchise_id = f.id) OR ((coupons.company_id)::text IN ( SELECT m.id            FROM merchants m           WHERE (m.region_id = f.region_id)))))))
//   Policy "franchisee_manage_coupons_ext" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND ((coupons.franchise_id = f.id) OR ((coupons.company_id)::text IN ( SELECT merchants.id            FROM merchants           WHERE ((merchants.region_id = f.region_id) OR (merchants.region = f.region))))))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND ((coupons.franchise_id = f.id) OR ((coupons.company_id)::text IN ( SELECT merchants.id            FROM merchants           WHERE ((merchants.region_id = f.region_id) OR (merchants.region = f.region))))))))
...
//     USING: ((company_id = (auth.uid())::text) OR (franchise_id = (auth.uid())::text) OR ((affiliate_id)::text = (auth.uid())::text) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))))
//     WITH CHECK: ((company_id = (auth.uid())::text) OR (franchise_id = (auth.uid())::text) OR ((affiliate_id)::text = (auth.uid())::text) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'super_admin'::text]))))))
...
//     USING: true
//     WITH CHECK: true
//   Policy "authenticated_select_own_franchise" (SELECT, PERMISSIVE) roles={authenticated}
...
//     USING: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND (merchants.region_id = f.region_id))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM (profiles p      JOIN franchises f ON ((f.email = p.email)))   WHERE ((p.id = auth.uid()) AND (p.role = 'franchisee'::text) AND (merchants.region_id = f.region_id))))
...
//   Policy "merchants_read_company_profiles" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((company_id IS NOT NULL) AND (company_id IN ( SELECT p2.company_id    FROM profiles p2   WHERE ((p2.id = auth.uid()) AND (p2.role = ANY (ARRAY['merchant'::text, 'shopkeeper'::text]))))))

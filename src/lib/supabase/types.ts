...
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'franchisee'::text) AND ((ad_campaigns.company_id)::text IN ( SELECT merchants.id            FROM merchants           WHERE (merchants.franchise_id = profiles.franchise_id))))))
